...
#define DAEMONIZE(a) daemonize(); \
	do { if (a) \
		strscpy(current->comm, a, MIN(sizeof(current->comm), (strlen(a) + 1))); \
	} while (0);
...
)
{
	...
	strscpy(extra, flag, sizeof(extra));
...
)
{
	...
	switch (phytype) {
		case WLC_PHY_TYPE_A:
			strscpy(cap, "a", sizeof(cap));
			break;
		case WLC_PHY_TYPE_B:
			strscpy(cap, "b", sizeof(cap));
			break;
		case WLC_PHY_TYPE_G:
			if (band[0] >= 2)
				strscpy(cap, "abg", sizeof(cap));
			else
				strscpy(cap, "bg", sizeof(cap));
			break;
		case WLC_PHY_TYPE_N:
			if (band[0] >= 2)
				strscpy(cap, "abgn", sizeof(cap));
			else
				strscpy(cap, "bgn", sizeof(cap));
			break;
	}
done:
